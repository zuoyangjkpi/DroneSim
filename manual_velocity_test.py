...
class ManualVelocityTester(Node):
    def __init__(self):
        ...
        self.declare_parameter('position_gain', 0.5)  # k for vx, vy
        ...
        self.state = 'INIT'  # INIT, TAKEOFF, FLYING, HOLDING
    ...
    def control_loop(self):
        ...
        if self.state == 'INIT':
            ...
            if distance_xy < self.pos_tol and abs(dz) < self.pos_tol and abs(dyaw) < self.yaw_tol:
                self.get_logger().info('✅ Target reached! Entering HOLD mode')
                self.state = 'HOLDING'

        elif self.state == 'HOLDING':
            # Just publish zero velocity to hold position
            # Gazebo plugin should maintain current position
            ...
            if self._hold_counter >= 40:  # 2 seconds at 20 Hz
                current_yaw = self.quaternion_to_yaw(self.current_pose.orientation)
                # Calculate drift from target
                drift_x = self.current_pose.position.x - self.target_pose.pose.position.x
                drift_y = self.current_pose.position.y - self.target_pose.pose.position.y
                drift_z = self.current_pose.position.z - self.target_pose.pose.position.z
                drift_yaw = self.wrap_angle(current_yaw - self.target_yaw)
                drift_dist = math.sqrt(drift_x**2 + drift_y**2)

                self.get_logger().info(
                    f'⏸️  HOLDING at ({self.current_pose.position.x:.2f}, '
                    f'{self.current_pose.position.y:.2f}, {self.current_pose.position.z:.2f}) | '
                    f'Yaw: {math.degrees(current_yaw):.1f}° | '
                    f'Drift: ({drift_x:.2f}, {drift_y:.2f}, {drift_z:.2f}, yaw: {math.degrees(drift_yaw):.1f}°) | '
                    f'Dist: {drift_dist:.2f}m'
                )
                self._hold_counter = 0
